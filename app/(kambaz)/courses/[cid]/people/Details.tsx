  ...
  return (
    <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
      ...
      <div>
        {!editing && (
          <div className="wd-email"
               onClick={() => setEditing(true)}>
            <b>Roles:</b>      <span className="wd-roles">         {user.role}         </span></div>)}
        {user && editing && (
            <select className="w-50 wd-edit-name"
            defaultValue={`${user.role}`}
            onChange={(e) => setRole(e.target.value)}
            onKeyDown={(e) => {if (e.key === "Enter") { saveUser(); }}}>
                <option value="STUDENT">Students</option> <option value="TA">Assistants</option> 
                <option value="FACULTY">Faculty</option> <option value="ADMIN">Administrators</option>
            </select>)}
      </div> <br />
      ...
      <div>
        ...
        <b>Email:</b>  <span className="wd-total-activity"> {user.email} </span> </div>)}
        {user && editing && (
            <FormControl className="w-50 wd-edit-name" type="email"
